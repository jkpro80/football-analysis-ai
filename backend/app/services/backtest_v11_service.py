...
class BacktestV11Service:
    """
    يشغّل Prediction V11 على مباريات منتهية ويقارن التوقعات بالنتائج الفعلية.

    المبادئ:
    - يستخدم MatchAnalysisPipeline نفسه المستخدم في الإنتاج.
    - لا يحفظ Features أو Predictions أثناء الاختبار.
    - لا يغيّر منطق Prediction V6 أو Prediction V11.
    - يعزل فشل أي مباراة عن بقية عينة الاختبار.
    """

    def __init__(self, db: Session) -> None:
        self.db = db
        self.pipeline = MatchAnalysisPipelineV11(db=db)
    ...
    def _evaluate_match(
        ...
        btts_market = self._mapping(prediction.get("btts"))
        ...
        totals_market = self._mapping(prediction.get("totals"))
    ...
    def _double_chance_is_correct(
        ...
        allowed_results = {
            ...
            "home_or_draw_1x": {"home_win", "draw"},
            "1x": {"home_win", "draw"},
            "away_or_draw": {"away_win", "draw"},
            "draw_or_away_x2": {"away_win", "draw"},
            "x2": {"away_win", "draw"},
            "home_or_away": {"home_win", "away_win"},
            "home_or_away_12": {"home_win", "away_win"},
