...
export default function MatchIntelligence({locale,available,data}:Props){
 ...
 return <section className="rounded-2xl border border-cyan-400/20 bg-[#040a18] p-4 sm:p-5"><header className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[14px] font-black uppercase tracking-[.18em] text-cyan-400">PRO INTELLIGENCE</p><h2 className="mt-1 text-2xl font-black text-white">{text.title}</h2><p className="mt-1 text-[14px] text-slate-400">{text.subtitle}</p></div><span className="rounded-full border border-cyan-400/15 px-3 py-1.5 text-[14px] font-black text-cyan-300">AI CONTEXT</span></header>
  ...
  <div className="mt-3 rounded-2xl border border-slate-800 bg-slate-950/30 p-4"><h3 className="text-lg font-black text-white">{text.weather}</h3>{w.available?<div className="mt-3 grid gap-2 sm:grid-cols-3 xl:grid-cols-6">{[[text.temperature,w.temperature!=null?`${w.temperature.toFixed(1)} °C`:"—"],[text.feelsLike,w.feels_like!=null?`${w.feels_like.toFixed(1)} °C`:"—"],[text.humidity,w.humidity!=null?`${w.humidity.toFixed(0)}%`:"—"],[text.wind,w.wind_speed!=null?`${w.wind_speed.toFixed(1)} m/s`:"—"],[text.attackImpact,percentage(w.attack_factor)],[text.fatigueImpact,percentage(w.fatigue_factor)]].map(([l,v])=><div key={l} className="rounded-xl border border-slate-800 bg-slate-900/35 px-3 py-2.5"><div className="text-[13px] font-bold text-slate-500">{l}</div><strong className="mt-1 block text-base font-black text-cyan-300" dir="ltr">{v}</strong></div>)}</div>:<p className="mt-2 text-sm text-slate-500">{text.noWeather}</p>}{w.description&&<p className="mt-2 text-sm text-slate-400">{translateWeather(w.description,locale)}</p>}</div>
  {data.warnings.length>0&&<div className="mt-3 rounded-2xl border border-amber-400/20 bg-amber-400/[.025] p-4"><h3 className="text-base font-black text-amber-300">{text.warnings}</h3><ul className="mt-2 grid gap-2 md:grid-cols-2">{data.warnings.map((x,i)=><li key={`${x}-${i}`} className="text-[14px] leading-5 text-slate-300">• {translateWarning(x,locale)}</li>)}</ul></div>}
 </section>;
}
